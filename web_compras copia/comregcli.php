...
if ($_SERVER["REQUEST_METHOD"]=="POST") {
    ...
    try {
        ...
        if (empty($_POST["NIF"])) { //comprueba si el input está vacía
            ...
            if (!preg_match("/^[0-9]{8}[A-Z]$/",$NIF)) { //comprueba si cumple el formato
                ...
                if ($count_nif>0) {
                    ...
                    $nom_cliente = limpieza($nom_cliente);
                    $contrasenia = limpieza($ape_cliente);
                    $contrasenia = strrev($contrasenia); //al revés
                    echo "Your user: " . $nom_cliente . "<br>";
                    echo "Your password: " . $contrasenia . "<br>";

                    $stmt3 = $conn->prepare("INSERT INTO USUARIOS (USUARIO,CONTRASENIA,NIF) VALUES (:USUARIO,:CONTRASENIA,:NIF)");
                    $stmt3 -> bindParam(':USUARIO',$nom_cliente);
                    $stmt3 -> bindParam(':CONTRASENIA',$contrasenia);
                    $stmt3 -> bindParam(':NIF',$NIF);
...
// CREATE TABLE USUARIOS (USUARIO VARCHAR(20)NOT NULL PRIMARY KEY, CONTRASENIA VARCHAR(20) NOT NULL);
//ALTER TABLE USUARIOS ADD CONSTRAINT FK_NIF_CLI FOREIGN KEY (NIF) REFERENCES CLIENTE(NIF);  
